...
int findSubLenAtStrPosWithQmark(const char str[], const char pattern[], int strPos = 0, int patPos = 0)
{
    ...
    if (str[strPos] == pattern[patPos]) {
        int next = findSubLenAtStrPosWithQmark(str, pattern, strPos +1, patPos+1);  
        return (next != NOT_FOUND) ? next+1 : NOT_FOUND;
    }
    if (pattern[patPos] == QMARK) {
        int next = (str[strPos] != NULL_CHAR) ? findSubLenAtStrPosWithQmark(str, pattern, strPos +1, patPos+1) : NOT_FOUND;
        int zero = findSubLenAtStrPosWithQmark(str, pattern, strPos, patPos+1);  
        int ans = NOT_FOUND;
        if (next != NOT_FOUND) ans = max(ans, next+1);
        if (zero != NOT_FOUND) ans = max(ans, zero);
        return ans;
    }
...
{
    if (pattern[patPos] == NULL_CHAR) return 0;
    if (str[strPos] == pattern[patPos]) {
        int next = findSubLenAtStrPosWithPercent(str, pattern, strPos +1, patPos+1);  
        return (next != NOT_FOUND) ? next+1 : NOT_FOUND;
    }
    if (pattern[patPos] == PERCENT) {
        int next = (str[strPos] != NULL_CHAR) ? findSubLenAtStrPosWithPercent(str, pattern, strPos +1, patPos) : NOT_FOUND;
        int zero = findSubLenAtStrPosWithPercent(str, pattern, strPos, patPos+1);  
        int ans = NOT_FOUND;
        if (next != NOT_FOUND) ans = max(ans, next+1);
        if (zero != NOT_FOUND) ans = max(ans, zero);
        return ans;
...
int findSubLenAtStrPosWithTask3(const char str[], const char pattern[], int strPos = 0, int patPos = 0)
{
    if (pattern[patPos] == NULL_CHAR) return 0;
    if (str[strPos] == pattern[patPos]) {
        int next = findSubLenAtStrPosWithTask3(str, pattern, strPos +1, patPos+1);  
        return (next != NOT_FOUND) ? next+1 : NOT_FOUND;
    }
    if (pattern[patPos] == PERCENT) {
        int next = (str[strPos] != NULL_CHAR) ? findSubLenAtStrPosWithTask3(str, pattern, strPos +1, patPos) : NOT_FOUND;
        int zero = findSubLenAtStrPosWithTask3(str, pattern, strPos, patPos+1);  
        int ans = NOT_FOUND;
        if (next != NOT_FOUND) ans = max(ans, next+1);
        if (zero != NOT_FOUND) ans = max(ans, zero);
        return ans;
    ...
    if (pattern[patPos] == DOT) { 
        int next = (str[strPos] != NULL_CHAR) ? findSubLenAtStrPosWithTask3(str, pattern, strPos +1, patPos+1) : NOT_FOUND;
        int ans = NOT_FOUND;
        if (next != NOT_FOUND) ans = max(ans, next+1);
        return ans;
    }
...
{ 
    ...
    if (start == 0 && pattern[start] == CARET) { 
        int idx = matchSub(str, pattern+1, length, start); 
        if (idx == 0) {
            return idx;
        } else {
            length =0;
    ...
    if (str[start] == NULL_CHAR)
        return NOT_FOUND;
    int testLength = findSubLenAtStrPosWithTask3(str, pattern, start);
    if (testLength != NOT_FOUND) {
        length = testLength;
        return start;
    }
