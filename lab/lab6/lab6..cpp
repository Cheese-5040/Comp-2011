...
bool is_valid(int x, int y, int width, int height) {
    ...
    // check if y exceeds the boundary of height or less than zero 
...
bool find_safe_path(char maze[], bool path[], int x, int y, int exit_x, int exit_y, int width, int height) {
    ...
    while (is_exit(x,y,exit_x,exit_y)== false){
        // check if it is valid, is a wall, or is dangerous area
        // prioritise moving to the right, then moving down
      
        if (is_valid(x,y,width, height)== true ){  
            ++x;// move right as primary direction
            if (x== width){
            ++y;
            }
            else if(is_wall(maze, x, y, width, height)==false){
                ++y; //if hit wall, go down (cannot go right)
            }
            else if (is_dangerous(maze,x,y,width,height)==false){
                ++y;//if dangerous area, go down
            }
            find_safe_path(maze, path, x, y, exit_x, exit_y, width, height);
