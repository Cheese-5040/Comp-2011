...
void print_addition_carry_out(int a, int b)
{
    ...
    for (int j = 0; j < larger_min_bits; j++)
    {
        ...
        for (int i = 0; i < larger_min_bits - j; i++)
        {
            ...
            if (a_bit == 1 && b_bit == 1)
            {
                (carry_over_bit == 0) ? return_value = 1, carry_over_bit = 1 : (carry_over_bit == 1) ? return_value = 1, carry_over_bit = 1 : carry_over_bit = 2;
            }
            else if (a_bit == 1 && b_bit == 0)
            {
                (carry_over_bit == 0) ? return_value = 0 : (carry_over_bit == 1) ? return_value = 1, carry_over_bit = 1 : carry_over_bit = 2;
            }
            ...
            {
                (carry_over_bit == 0) ? return_value = 0 : (carry_over_bit == 1) ? return_value = 0, carry_over_bit = 0 : carry_over_bit = 2;
            }
...
int main()
{
    while (true)
    {
        ...
        cout << "Enter number B: ";
        cin >> b;
        print_2s_complement_representation(a);
        print_2s_complement_representation(b);
